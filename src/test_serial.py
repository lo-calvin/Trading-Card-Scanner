...
def process_card(mask, bbox, img, track_id, ret, results):
    x_min, y_min, x_max, y_max = get_bbox_corner(bbox, img)
...
def process_all_cards(masks, bboxs, img, track_ids, ret):
    tasks = []
    results = []

    # Loop over masks and create asynchronous tasks for each card
    for i in range(len(masks)):
        task = process_card(masks[i], bboxs[i], img, track_ids[i], ret, results)
        tasks.append(task)
...
def main():
    ...
    process_all_cards(masks, bboxs, img, track_ids, ret)
